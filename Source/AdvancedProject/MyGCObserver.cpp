...
void AMyGCObserver::Tick(float DeltaTime)
{
	...
	GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::Green,
	                                 FString::Printf(
		                                 TEXT("Safe(UPROPERTY) -> if : %s, IsValid : %s"),
		                                 bSafeIf ? TEXT("True") : TEXT("False"),
		                                 bSafeValid ? TEXT("True") : TEXT("False")));
	GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::Red,
	                                 FString::Printf(
		                                 TEXT("Danger(UPROPERTY) -> if : %s, IsValid : %s"),
		                                 bDangerIf ? TEXT("True") : TEXT("False"),
		                                 bDangerValid ? TEXT("True") : TEXT("False")));
}
